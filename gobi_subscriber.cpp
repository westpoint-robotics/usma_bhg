...
int main(int argc, char **argv)
{
    ...
    //                frameBuffer = new word[frameSize / 2];
                    ...
                    else
                    {
                        ...
                        utc_tm = *gmtime(&tt);
                        local_tm = *localtime(&tt);
                        dateTime = to_string(local_tm.tm_year + 1900) + to_string(local_tm.tm_mon + 1) + to_string(local_tm.tm_mday) + '_' 
                                        + to_string(local_tm.tm_hour) + to_string(local_tm.tm_min) + to_string(local_tm.tm_sec) + "_" + to_string(static_cast<unsigned>(tp / milliseconds(1)));
